        ...
        if db:
            try:
                ...
                if docs:
                    ...
                    all_users = users_ref.limit(5).get()
                    print(f"🔍 Total users in database: {len(all_users)}")
                    for user_doc in all_users:
                        user_data = user_doc.to_dict()
                        print(f"🔍 Existing user: {user_doc.id} - email: {user_data.get('user_email', 'NO EMAIL FIELD')}")
            ...
            print("⚠️  Firestore not available - using session-based login")
            # Fallback to email-based user_id for session-only mode
...
def signup():
    ...
    stripe_publishable_key = os.environ.get('STRIPE_PUBLISHABLE_KEY')
    return render_template("signup.html", stripe_publishable_key=stripe_publishable_key)

@application.route("/signup-and-subscribe", methods=["POST"])
def signup_and_subscribe():
    """Handle user signup and subscription creation in one flow"""
    try:
        data = request.get_json()
        ...
        if not all([user_name, user_email, plan, payment_method_id]):
        ...
        subscription = stripe.Subscription.create(
            ...
            metadata={'user_email': user_email, 'user_name': user_name, 'plan': plan}
        )
        
        if subscription.status in ['active', 'trialing']:
            # Generate a user ID (you might want to use Firebase Auth or another method)
            user_id = f"user_{customer.id}_{int(datetime.now().timestamp())}"
            
            # Save subscription to Firestore
            start_date = str(datetime.now())
            
            print(f"🔍 New user signup: {user_name} ({user_email})")
